...
# ECSサービス
module "online_ecs_service" {
  source = "git@github.com:biglobe-isp/terraform-aws-ecs-service.git?ref=tags/v0.3.0"

  # オンライン／バッチ固有部分
  name             = local.online_ecs_name
  target_group_arn = aws_lb_target_group.online.id
  ...
  container_definitions = jsonencode([
    {
      ...
      environment       = concat(local.environment, local.online_environment)
      essential         = true
      ...
      logConfiguration = {
        ...
        options = {
          ...
          region            = local.region
          log_group_name    = module.online_bows_cloudwatch_logs.privileged_name
          log_stream_prefix = "${local.online_component_type}-"
        }
      }
    ...
    },
    {
      ...
      environment           = concat(local.fluentbit_environment, local.online_fluentbit_environment)
      essential             = true
      ...
      logConfiguration = {
        logDriver = "awslogs"
        options = {
          awslogs-group         = module.online_bows_cloudwatch_logs.log_router_name
          awslogs-region        = local.region
          awslogs-stream-prefix = local.online_component_type
        }
      }
...
# CloudWatch Logs
module "online_bows_cloudwatch_logs" {
  source = "git@github.com:biglobe-isp/terraform-aws-bows-cloudwatch-logs.git?ref=tags/v0.2.0"

  # オンライン／バッチ固有部分
  name_prefix = "/ecs/${local.component_name}/${local.online_component_type}"
...
# ECS用のALBリスナールール
resource "aws_lb_listener_rule" "online" {
  ...
  action {
    type             = "forward"
    target_group_arn = aws_lb_target_group.online.arn
  }

  condition {
    host_header {
      values = [aws_route53_record.online.fqdn]
    }
  }
}

# ECS用のターゲットグループ
resource "aws_lb_target_group" "online" {
  name     = local.online_ecs_name
...
# ECS用のALIASレコード
resource "aws_route53_record" "online" {
  zone_id = local.bows_zone_id
  name    = "${local.online_component_type}.${local.domain_name}"
  type    = "A"
